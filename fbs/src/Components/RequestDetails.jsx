...
const RequestDetails = () => {
  ...
  const [selectedNameForConfirmation, setSelectedNameForConfirmation] = useState(null);
  ...
  const fetchRequest = async () => {
    try {
      ...
      setSubmission(req);
   const savedStage = localStorage.getItem("activeStage_" + id);
const completedStage = getCompletedStageIndex(req);
const defaultStage = getActiveStageIndex(req); // server-driven

if (savedStage !== null) {
  const saved = Number(savedStage);

  // 👇 if saved stage is *behind* server progress, override with server
  if (saved < defaultStage) {
    setActiveStage(defaultStage);
    localStorage.setItem("activeStage_" + id, defaultStage);
  } else {
  ...
  {/* Grey base line */}
  <div className="absolute top-6 left-[6%] right-[6%] h-1 bg-gray-300"></div>

  {/* Primary progress line */}
  <div
    className="absolute top-6 left-[6%] h-1 transition-all duration-500"
    style={{
      backgroundColor: primaryColor,
      width: `${Math.min(
  (activeStage / (stages.length - 1)) * 88,
  88
)}%`
, // 88% = (100% - left 6% - right 6%)
    }}
  ></div>
...
const isActive = i === activeStage;
      ...
      return (
        <div key={i} className="flex flex-col items-center relative">
          ...
          <div
            ...
            style={{
              backgroundColor:
                isCompleted || isActive ? primaryColor : "#d1d5db",
          ...
          <span
            ...
            style={{
              color: isCompleted || isActive ? primaryColor : "#9ca3af",
            }}
          >
            {stage}
          </span>
  ...
  <StageBlock title="Admin Reviewed Names">
    {submission.suggestedToCustomer?.length > 0 && (
      <div className="mb-4 flex flex-wrap gap-3">
        {submission.suggestedToCustomer.map((name, idx) => {
          const isSelected = submission.selectedName === name;
          return (
            <div key={idx} className="relative">
              <button
                onClick={() => !submission.selectedName && setSelectedNameForConfirmation(name)}
                ...
                style={{
                  backgroundColor: isSelected
                    ? "#a1a1a1" // Grey for selected
                    : "green",   // Green for available names
                }}
              >
                {name}
              </button>
    ...
    {/* Confirmation Modal */}
    {selectedNameForConfirmation && (
      <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4">
          <h3 className="text-xl font-semibold mb-4">Confirm Your Selection</h3>
          <p className="mb-4">
            Are you sure you want to select <strong>{selectedNameForConfirmation}</strong> as your final brand name?
          </p>
          <p className="text-sm text-gray-600 mb-6 italic">
            This selection cannot be changed once confirmed.
          </p>
          <div className="flex justify-end space-x-4">
            <button
              onClick={() => setSelectedNameForConfirmation(null)}
            ...
            <button
              onClick={() => {
                handleSelectFinalName(selectedNameForConfirmation);
                setSelectedNameForConfirmation(null);
              }}
...
function getCompletedStageIndex(sub) {
  if (sub.customerSignedDocUrl) return 4; // Stage 4 done
  if (sub.adminDocumentUrl) return 3;     // Stage 3 done
  if (sub.paymentCompleted) return 2;     // Stage 2 done
  if (sub.selectedName) return 1;         // Stage 1 done
  if (sub.suggestedToCustomer?.length > 0) return 0; // Stage 0 done
  return -1; // nothing completed yet
}

function getActiveStageIndex(sub) {
  const completed = getCompletedStageIndex(sub);

  // If everything is completed, stay at last stage
  if (completed >= stages.length - 1) return completed;

  // Otherwise, move to next pending stage
  return completed + 1;
}
